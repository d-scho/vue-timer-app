...
import DragIcon from '@/components/generics/DragIcon.vue';
...
<template>
    <VueDraggableNext
        ...
        tag="ul"
        class="timers"
        @change="setStoredTimers(timers)"
        ghost-class="ghost"
        @dragstart="isAnimationEnabled = false"
        @dragend="isAnimationEnabled = true"
        handle=".drag-handle"
    >
        <TransitionGroup :name="isAnimationEnabled ? 'slide' : ''">
...
.slide-leave-active {
    margin-top: calc(
        (68px + 0.4rem) * -1
    ); // you are supposed to set position: absolute here, but that didn't work
}
</style>
